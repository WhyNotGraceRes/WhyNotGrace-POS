...
export function ItemCustomizeDialog({ item, onClose }: { item: MenuItemOut | null; onClose: () => void }) {
  const { t } = useTranslation();
  const addLine = useCartStore((s) => s.addLine);

  const defaultVariantId = useMemo(() => {
    if (!item || item.variants.length === 0) return undefined;
    return item.variants.find((v) => v.is_default && v.is_active)?.id ?? item.variants.find((v) => v.is_active)?.id;
  }, [item]);
  ...
  const unitEstimate = item.base_price + (selectedVariant?.price_delta ?? 0);
              ...
              .map((v) => (
                ...
                >
                  ...
                  <span className="font-medium text-slate-600">
                    {formatCurrency(item.base_price + v.price_delta)}
                  </span>
                </label>
              ))}
          </div>
