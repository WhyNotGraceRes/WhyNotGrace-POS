...
export function MenuItemCard({ item, onSelect }: { item: MenuItemOut; onSelect: (item: MenuItemOut) => void }) {
  ...
  return (
    <button
      ...
      className={cn(
        "group relative flex flex-col overflow-hidden rounded-card border border-slate-200 bg-white text-left transition-shadow",
    ...
    >
      {item.image_url ? (
        ...
        <div className="flex h-24 w-full items-center justify-center bg-slate-100 text-2xl font-bold text-slate-300">
          {item.name[0]}
        </div>
      )}
      ...
      <div className="flex flex-1 flex-col gap-1 p-3">
        <div className="flex items-start gap-1.5">
          ...
          <span className="line-clamp-2 text-sm font-semibold leading-snug text-slate-800">{item.name}</span>
        </div>
        <div className="mt-auto flex items-center justify-between pt-1">
          ...
          {(hasVariants || item.option_groups.length > 0) && (
            <span className="text-[10px] font-medium text-slate-400">{t("pos.customizable")}</span>
          )}
        </div>
      </div>
    </button>
