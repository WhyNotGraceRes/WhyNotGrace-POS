...
export function CounterRulesSection() {
  ...
  return (
    <Card className="space-y-4 p-5">
      <div>
        <h2 className="text-sm font-semibold text-stone-900">{t("counterRules.title")}</h2>
        <p className="mt-0.5 text-xs text-stone-500">{t("counterRules.subtitle")}</p>
      </div>

      {series && (
        <div className="rounded-lg bg-stone-50 px-3 py-2 text-xs">
          <span className="text-stone-500">{t("counterRules.nextInvoice")}</span>{" "}
          <span className="font-mono font-medium text-stone-900">{series.next_number}</span>
      ...
      {groups.map(([group, items]) => (
        <div key={group} className="rounded-lg border border-stone-200">
          <div className="border-b border-stone-100 px-3 py-2 text-xs font-medium uppercase tracking-wide text-stone-500">
            {t(`counterRules.group.${group}`, group)}
          </div>

          {items.map((toggle) => (
            <div key={toggle.key} className="border-b border-stone-50 px-3 py-3 last:border-0">
                  ...
                  <p className="flex items-center gap-1.5 text-sm font-medium text-stone-800">
                    ...
                    {!toggle.owner_editable && (
                      <span
                        ...
                        className="inline-flex items-center gap-1 rounded bg-stone-100 px-1.5 py-0.5 text-[10px] font-normal text-stone-500"
                      >
